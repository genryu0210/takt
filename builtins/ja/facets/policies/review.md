...
| 機能 PR で CHANGELOG / RELEASE_NOTES が更新されていない | **REJECT 禁止**（リリース時に専用ツールがコミット履歴から自動生成するため） |
| 機能 PR で CHANGELOG / RELEASE_NOTES に新エントリが追加されている | **REJECT 可**（該当エントリの削除を要求する。理由: リリース済みセクションへの追記はリリース履歴を歪める。リリースツール側で自動生成されるべき） |
...
- CHANGELOG / RELEASE_NOTES / MIGRATION の更新を機能 PR の REJECT 理由にしてはならない。これらはリリース時に専用ツールが生成する
