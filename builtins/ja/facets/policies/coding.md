...
- **CHANGELOG / RELEASE_NOTES / MIGRATION の手動編集禁止** - 機能 PR でこれらのリリースノート系ファイルを更新しない。リリース時に専用ツール（例: `/release` スキル）がコミット履歴から自動生成する。タスク指示書に「changelog 更新」が含まれていても、これらに該当する場合は対象外として扱う（リリース PR で明示的に指示された場合のみ例外）
