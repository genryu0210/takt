...
| Feature PR has not updated CHANGELOG / RELEASE_NOTES | **REJECT prohibited** (a dedicated release tool generates entries from commit history at release time) |
| Feature PR adds new entries to CHANGELOG / RELEASE_NOTES | **REJECT allowed** (request removal of those entries; rationale: appending to a released section distorts release history; the release tool generates entries) |
...
- Do not REJECT a feature PR for not updating CHANGELOG / RELEASE_NOTES / MIGRATION. These are generated by the release tool
